...
class CMF_Free {
    ...
    public static function init() {
        ...
        add_filter('cmf_meta_after_content', [__CLASS__, 'cmf_copy_to_clipboard'], 10, 3);
    }
    ...
    public static function includeFiles() {
        ...
        include_once CMF_PLUGIN_DIR . "wizard.php";
    }
...
//        wp_enqueue_script('jquery-ui-tooltip');
    ...
    public static function cmf_render_definition_box($post) {
        ...
        $metabox_description .= '<div class="cmf-pro-only"><p class="cmbd-import-footnotes-description">';
        $metabox_description .= 'Plugin supports quick and simple import footnotes data lists from .csv files type. &nbsp';
        $metabox_description .= 'You can download <span class="disabled-link">CSV Sample File</span> &nbsp;';
        $metabox_description .= 'and read ho to use it <span class="disabled-link">here</span>. &nbsp';
        $metabox_description .= 'Please fill it by own data before import.</p><br />';
        $metabox_description .= '<input type="file" accept=".csv" disabled="disabled" />';
        $metabox_description .= '<input id="js-import-footnotes" type="submit" disabled="disabled" value="Import from CSV" class="button button-primary" />';
        $metabox_description .= '<br/></div><br/>';
...
<style>
            ...
            .cmf_shortcode .tooltiptext::after {
                content: "";
                position: absolute;
                top: 100%;
                left: 50%;
            ...
            .cmf_shortcode:hover .tooltiptext {
                visibility: visible;
                opacity: 1;
            }
            </style>
            <script>
            function cmf_shortcode_on(id) {
...
//            '_builtin' => false
        ...
        public static function cmf_copy_to_clipboard($block_arr, $existingDefinitions, $i) {
        if(isset($existingDefinitions['ids'][$i]) && !empty($existingDefinitions['ids'][$i])) {
            $id = $existingDefinitions['ids'][$i];
        } else {
            $id =  (bool)$i? $i :  1 + $i;
        }
        ...
        ob_start();
        ?>
        <div class="cmf_shortcode">
            <input id="cmf_shortcode_<?php echo $id; ?>" class="cmf_shortcode_input" type="text" readonly="readonly" value="[cm_simple_footnote id=<?php echo $id; ?>]"/>
            <button class="cmf_shortcode_button" onclick="cmf_shortcode_on('cmf_shortcode_<?php echo $id; ?>'); return false;" onmouseout="cmf_shortcode_off('cmf_shortcode_<?php echo $id; ?>')">
                <span class="tooltiptext" id="cmf_shortcode_<?php echo $id; ?>_tooltip">Copy to clipboard</span>
                Copy shortcode
            </button>
        </div>
        <?php
