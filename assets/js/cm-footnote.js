(function ($) {
    $(document).ready(function () {
        ...
        $(document).on('click', 'a#cm_footnote_add_new_definition', function () {
            ...
            // Preparing dashicons edit block

            let showdasdiv = '<div class="cm-foot-settings-flex-block cm-foot-meta-dashicons-block ">';
            showdasdiv += '<button class="button dashicon-button_' + x + '  ws_select_icon" title="Select icon" data-id="' + x + '" disabled>';
            showdasdiv += '<div class="icon16 dashicons  render-dash-' + x + '"><span class="selct-icon-' + x + ' dashicons dashicons-edit"></span></div>';
            showdasdiv += '</button>'
            showdasdiv += '<button class="remve-icon" data-id="' + x + '" title="Remove the icon" disabled>';
            showdasdiv += '<span class="remv-icon-' + x + ' dashicons dashicons-no-alt"></span>';
            showdasdiv += '</button></div>';

            let copylink = '<div class="cmf_shortcode">';
            copylink += '<input id="cmf_shortcode_' + x + '" class="cmf_shortcode_input" type="text" readonly="readonly" value="[cm_simple_footnote id=' + x + ']">';
            copylink += '<button disabled class="cmf_shortcode_button" onclick="cmf_shortcode_on(\'cmf_shortcode_' + x + '\'); return false;" onmouseout="cmf_shortcode_off(\'cmf_shortcode_' + x + '\')">';
            copylink += '<span class="tooltiptext" id="cmf_shortcode_' + x + '_tooltip">Copy to clipboard</span>Copy shortcode</button></div>';
            ...
            htmlOut += copylink;
            htmlOut += '</div>';
            htmlOut += showdasdiv; // Add dash icons block
            htmlOut += '<div class="cm_footnote_definitions_td_extlink">';
            htmlOut += '<textarea  name="cmf_extlink" class="cm_footnote_definitions_row_extlink" rows="1" style="resize: none;" disabled></textarea>';
            htmlOut += '<input type="text" placeholder="Link label(optional)" name="cm_extlink_label" class="cm_footnote_definitions_row_extlink_label" disabled>';
            htmlOut += '</div>';
            htmlOut += '<div>';
